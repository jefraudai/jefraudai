...
def split_data(data, test_size=0.2, random_state=42, target_column=None):
    ...
    print("Aperçu des premières lignes de data_clean:")
    ...
    print(f"Colonne cible utilisée (paramètre): {target_col}")
    ...
    if target_col:
        ...
        X = data_clean.iloc[:, :-1]
        y = data_clean.iloc[:, -1]
        print(f"Colonne cible utilisée (fallback): {y.name}")

    print(f"Features shape: {X.shape}, Target shape: {y.shape}")
    print("Aperçu des premières lignes de X:")   
    print(X.head(2))
    print("Aperçu des premières lignes de y:")
    print(y.head(2))
    ...
    try:
        ...
        print(f"Attention: Conversion numérique partiellement échouée (Normal si AutoGluon): {e}")
